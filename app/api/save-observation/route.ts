...
export async function POST(request: Request) {
  try {
    ...
    const observationData = {
      ...
      // Rainfall - Handle both old and new formats
      // New format: timeSlots array
      rainfallTimeSlots: data.rainfall?.timeSlots
        ? data.rainfall.timeSlots
        : null,

      // Backward compatibility: store first slot in old fields
      ...
      rainfallSincePrevious:
        data.rainfall?.sincePrevious ||
        data.rainfall?.["since-previous"] ||
        null,
      rainfallDuringPrevious:
        data.rainfall?.duringPrevious ||
        data.rainfall?.["during-previous"] ||
        null,
      rainfallLast24Hours:
        data.rainfall?.last24Hours || data.rainfall?.["last-24-hours"] || null,
      rainfallType: data.rainfall?.rainfallType || null,
      isIntermittentRain:
        data.rainfall?.rainfallType === "intermittent" ||
...
export async function PUT(request: Request) {
  try {
    ...
    if (type === "weather") {
      ...
      if (canEdit && !(await canEditRecord(existing, session.user))) {
        return NextResponse.json(
          {
            success: false,
