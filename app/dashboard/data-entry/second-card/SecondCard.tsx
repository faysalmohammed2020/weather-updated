...
type WeatherObservationFormData = {
  ...
  rainfall: {
    timeSlots?: { id: string; timeStart: string; timeEnd: string }[]; // new
    "date-start"?: string;
    "date-end"?: string;
    "since-previous"?: string;
    "during-previous"?: string;
    "last-24-hours"?: string;
    rainfallType?: "continuous" | "intermittent" | ""; // new
  };
  wind: {
...
// OPTIONAL: slot structure validate
...
export default function SecondCardForm({ timeInfo }: { timeInfo: TimeInfo[] }) {
  ...
  const formik = useFormik<WeatherObservationFormData>({
    initialValues: {
      ...
      rainfall: {
        ...
        rainfallType: "", // ✅ new (RainfallTab auto-set করবে)
      },
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name.startsWith("low-cloud-")) {
      ...
      // rainfall mapping কেবল এই তিনটা রাখুন:
      name === "since-previous" ||
  ...
  async function handleSubmit(values: WeatherObservationFormData) {
    ...
    try {
      ...
      toast.success(data.message, {
        description: `Second Card Entry Saved Successfully`,
      });
                  ...
                  <div className="p-4 sm:p-6">
                        ...
                        <CardContent className="pt-4 sm:pt-6">
                          <div className="grid gap-4 sm:gap-6">
                            <SelectField
                              ...
                              options={cloudAmountOptions.map(
                                (opt) => opt.value
                              )}
                    ...
                    >
                      ...
                      >
                        ...
                        <CardContent className="pt-4 sm:pt-6">
                          {/* আপনার Rainfall ফিল্ডগুলো */}
                          <RainfallTab formik={formik} />
                        </CardContent>
                          ...
                          <div className="grid gap-4 sm:gap-6 md:grid-cols-2">
                            ...
                            <div className="grid gap-2">
                              ...
                              <Input
                                ...
                                max="360"
                                value={
                                  formik.values.wind["wind-direction"] || ""
                                }
                                onChange={handleInputChange}
                                className={cn(
                      ...
                      >
                        ...
                        <CardContent className="pt-4 sm:pt-6 space-y-6">
                          ...
                          <SectionCard
                            title="Rainfall"
                            icon={
                              <CloudRainIcon className="h-5 w-5 text-cyan-500" />
                            }
                              ...
                              {(formik.values.rainfall.timeSlots || []).map(
                                (s, i) => (
                                  <div key={s.id || i} className="text-sm">
                                    Slot {i + 1}: {s.timeStart || "--:--"} –{" "}
                                    {s.timeEnd || "--:--"} (UTC)
                                  </div>
                                )
                              )}
                              {(!formik.values.rainfall.timeSlots ||
                                formik.values.rainfall.timeSlots.length ===
                                  0) && (
    ...
    <div className="bg-gradient-to-r from-white to-gray-50 p-4 rounded-lg border border-gray-200">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <SelectField
          ...
          options={withDeselect(formOptions).map((opt) => opt.value)}
          optionLabels={withDeselect(formOptions).map((opt) =>
        ...
        <SelectField
          ...
          options={withDeselect(cloudAmountOptions).map((opt) => opt.value)}
          optionLabels={withDeselect(cloudAmountOptions).map(
            (opt) => opt.label
          )}
        ...
        {prefix !== "high-cloud" && (
          <SelectField
            ...
            options={withDeselect(cloudHeightOptions).map((opt) => opt.value)}
            optionLabels={withDeselect(cloudHeightOptions).map(
              (opt) => opt.label
            )}
        ...
        <SelectField
          ...
          options={withDeselect(cloudDirectionOptions).map((opt) => opt.value)}
          optionLabels={withDeselect(cloudDirectionOptions).map(
            (opt) => opt.label
          )}
...
function SignificantCloudSection({
  ...
  const amountValues = [NONE, ...SigcloudAmountOptions].map((o) =>
    "value" in o ? o.value : o
  );
  const amountLabels = [NONE, ...SigcloudAmountOptions].map((o) =>
    "label" in o ? o.label : String(o)
  );
