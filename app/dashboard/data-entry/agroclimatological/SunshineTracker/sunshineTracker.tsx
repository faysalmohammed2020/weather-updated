...
export default function SunshineTracker() {
  ...
  // Prevent duplicate entry for the selected date
  ...
  useEffect(() => {
    const fetchSunshineData = async () => {
      try {
        ...
        const normalized: DailyData[] = Array.isArray(raw)
          ? raw
          : Array.isArray(raw?.data)
            ? raw.data
            : (raw && typeof raw === "object" && "date" in raw && "hours" in raw)
              ? [raw as DailyData]
              : [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Client-side guard: do not allow overwrite
    if (hasEntryForSelectedDate) {
      alert("An entry already exists for this date. Further entries are not allowed.");
      return;
    }

    const newEntry: Omit<DailyData, "id" | "createdAt" | "updatedAt"> & { stationId?: string } = {
      date: currentDate,
      hours: [...currentHours],
      total: currentTotal,
    ...
    try {
      ...
      if (!res.ok) {
        const message = result?.error || `Submission failed (status ${res.status})`;
                    ...
                    {timeSlots.map((slot, index) => (
                      <div key={index} className="space-y-2">
                        ...
                        <Input
                          ...
                          placeholder={hasEntryForSelectedDate ? "Locked" : "0"}
