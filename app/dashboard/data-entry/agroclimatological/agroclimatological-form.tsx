...
export function AgroclimatologicalForm() {
          ...
          if (Array.isArray(soilData) && soilData.length > 0) {
            ...
            if (validEntries.length > 0) {
              ...
              const calculateAverage = (depths: number[]) => {
                ...
                return values.length > 0 ? (values.reduce((sum, val) => sum + val, 0) / values.length).toFixed(1) : ""
              }

              const avg0to20 = calculateAverage([5, 10, 20])
              const avg20to50 = calculateAverage([20, 30, 50])

              // Format like: 83.3 -> 833, 9.8 -> 098
              const formatToThree = (v: string) => v.replace(/\./g, "").padStart(3, "0").slice(-3)
              if (avg0to20) formik.setFieldValue("soilMoisture.depth0to20cm", formatToThree(avg0to20))
              if (avg20to50) formik.setFieldValue("soilMoisture.depth20to50cm", formatToThree(avg20to50))
            }
          }
        }
  ...
  // Restrict to at most 3 digits (no decimals) for Air Temperature fields
  ...
  // Restrict to at most 2 digits (no decimals) for Humidity & Evaporation tab
                          ...
                          <div key={height} className="space-y-3 p-4 bg-white rounded-lg border">
                            ...
                            <div className="space-y-2">
                              ...
                              <Input
                                ...
                                onChange={handleThreeDigitInput}
