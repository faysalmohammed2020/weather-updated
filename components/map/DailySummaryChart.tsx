  ...
  const fetchWeatherData = React.useCallback(async () => {
    try {
      ...
      const transformedData = data
        .map((item: any) => ({
          date: new Date(item.ObservingTime.utcTime)
            .toISOString()
            .split("T")[0],
    ...
    const days =
      timeRange === "7d"
        ...
        : timeRange === "30d"
          ? "Last 30 Days"
          : "Last 90 Days";
    const station = weatherData[0]?.stationName
      ? ` at ${weatherData[0].stationName}`
      : "";
    return `${days}${station}`;
  };
        ...
        <CardContent className="p-6">
          <div className="flex flex-col items-center justify-center h-[400px] gap-4 text-center">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Try adjusting the time range or check if data exists for the
                selected period.
              </p>
            </div>
          </div>
