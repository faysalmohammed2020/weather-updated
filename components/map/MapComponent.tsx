...
function WeatherAnimationWrapper({
  ...
  const stationSecondCard = secondCardData?.find(
    (entry: any) => entry.station.stationId === selectedStation.stationId
  );
  ...
  switch (activeWeatherLayer) {
    ...
    case "temperature":
      const temperature = meteoData?.maxMinTempCorrected
        ? Number(meteoData.maxMinTempCorrected)
        : 200;
      return (
    ...
    case "wind":
      const windSpeed = weatherObservation?.windSpeed
        ? Number(weatherObservation.windSpeed)
        : 0;
      const windDirection = weatherObservation?.windDirection
        ? Number(weatherObservation.windDirection)
        : 0;
      return windSpeed > 0 ? (
        <WindAnimation
          windSpeed={windSpeed}
    ...
    case "clouds":
      const cloudData = {
        totalCloudAmount: weatherObservation?.totalCloudAmount
          ? Number(weatherObservation.totalCloudAmount)
          : 0,
        lowCloudAmount: weatherObservation?.lowCloudAmount
          ? Number(weatherObservation.lowCloudAmount)
          : 0,
        mediumCloudAmount: weatherObservation?.mediumCloudAmount
          ? Number(weatherObservation.mediumCloudAmount)
          : 0,
        highCloudAmount: weatherObservation?.highCloudAmount
          ? Number(weatherObservation.highCloudAmount)
          : 0,
      };
    ...
    case "humidity":
      const humidity = meteoData?.relativeHumidity
        ? Number(meteoData.relativeHumidity)
        : 0;
      return humidity > 0 ? (
        <HumidityAnimation
          humidity={humidity}
    ...
    case "visibility":
      const visibility = meteoData?.horizontalVisibility
        ? Number(meteoData.horizontalVisibility)
        : 100;
      return (
        <VisibilityAnimation
    ...
    case "pressure":
      const pressure = meteoData?.stationLevelPressure
        ? Number(meteoData.stationLevelPressure)
        : 10130;
      return (
        <PressureAnimation
          pressure={pressure}
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 401) {
            console.warn("User not authenticated, stations will not load");
          } else {
