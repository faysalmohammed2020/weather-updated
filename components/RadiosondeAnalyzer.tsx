...
export default function RadiosondeAnalyzer() {
  ...
  // Enhanced export functions with proper text formatting
...
`
    ...
    decodedData.mandatoryLevels
      ...
      .forEach((level) => {
        txtContent += `${level.pressure.toString().padEnd(10)} ${(level.height?.toString() || "N/A").padEnd(10)} ${formatValue(level.temperature, "").padEnd(8)} ${formatValue(level.dewpoint, "").padEnd(8)} ${formatValue(level.windDirection, "", 0).padEnd(6)} ${formatValue(level.windSpeed, "", 0).padEnd(6)}\n`
      })
...
${"=".repeat(60)}`
  ...
  const exportMandatoryAsCsv = () => {
    ...
    decodedData.mandatoryLevels
      .filter((level) => level.pressure >= 100)
      .forEach((level) => {
        csv += `${decodedData.station},Day ${decodedData.date},${String(decodedData.time).padStart(2, "0")}:00,${level.pressure},${level.height ?? ""},${level.temperature ?? ""},${level.dewpoint ?? ""},${level.dewpointDepression ?? ""},${level.windDirection ?? ""},${level.windSpeed ?? ""}\n`
      })
  ...
  const exportMandatoryAsPdf = () => {
    ...
    doc.text(
      `Station: ${decodedData.station} | Date: Day ${decodedData.date} | Time: ${String(decodedData.time).padStart(2, "0")}:00 UTC`,
      margin,
      y,
    )
    y += 10
    ...
    decodedData.mandatoryLevels
      ...
      .forEach((level) => {
        ...
        const row = `${level.pressure.toString().padEnd(8)} ${(level.height?.toString() || "N/A").padEnd(10)} ${formatValue(level.temperature, "").padEnd(8)} ${formatValue(level.dewpoint, "").padEnd(9)} ${formatValue(level.windDirection, "", 0).padEnd(7)} ${formatValue(level.windSpeed, "", 0)}`
    ...
    if (decodedData.tropopause) {
      ...
      doc.setFont("helvetica", "normal")
      doc.text(
        `${decodedData.tropopause.pressure} mb | ${formatValue(decodedData.tropopause.temperature, "°C")} | Wind: ${decodedData.tropopause.windDirection}°/${decodedData.tropopause.windSpeed}kt`,
        margin,
        y,
      )
    }
...
${"=".repeat(65)}
${"Pressure".padEnd(10)} ${"Temp".padEnd(8)} ${"Dewpt".padEnd(8)} ${"Depr".padEnd(8)} ${"WDir".padEnd(6)} ${"WSpd".padEnd(6)} ${"Type".padEnd(10)}
${"(mb)".padEnd(10)} ${"(°C)".padEnd(8)} ${"(°C)".padEnd(8)} ${"(°C)".padEnd(8)} ${"(°)".padEnd(6)} ${"(kt)".padEnd(6)} ${" ".padEnd(10)}
${"-".repeat(65)}
`
    ...
    decodedData.significantLevels.forEach((level) => {
      ...
      txtContent += `${level.pressure.toString().padEnd(10)} ${formatValue(level.temperature, "").padEnd(8)} ${formatValue(level.dewpoint, "").padEnd(8)} ${formatValue(level.dewpointDepression, "").padEnd(8)} ${formatValue(level.windDirection, "", 0).padEnd(6)} ${formatValue(level.windSpeed, "", 0).padEnd(6)} ${levelType.padEnd(10)}\n`
    })
...
Surface Level Included: ${decodedData.significantLevels.some((l) => l.pressure === decodedData.surfacePressure) ? "Yes" : "No"}

Maximum Wind Speed: ${Math.max(...decodedData.significantLevels.map((l) => l.windSpeed || 0))} knots
Average Wind Speed: ${Math.round(decodedData.significantLevels.filter((l) => l.windSpeed !== null).reduce((sum, l) => sum + (l.windSpeed || 0), 0) / decodedData.significantLevels.filter((l) => l.windSpeed !== null).length)} knots
...
${"=".repeat(65)}`
  ...
  const exportSignificantAsCsv = () => {
    ...
    decodedData.significantLevels.forEach((level) => {
      const levelType = level.pressure === decodedData.surfacePressure ? "Surface" : "Significant"
      csv += `${decodedData.station},Day ${decodedData.date},${String(decodedData.time).padStart(2, "0")}:00,${level.pressure},${level.temperature ?? ""},${level.dewpoint ?? ""},${level.dewpointDepression ?? ""},${level.windDirection ?? ""},${level.windSpeed ?? ""},${levelType}\n`
    })
  ...
  const exportSignificantAsPdf = () => {
    ...
    decodedData.significantLevels.forEach((level) => {
      ...
      const row = `${level.pressure.toString().padEnd(8)} ${formatValue(level.temperature, "").padEnd(8)} ${formatValue(level.dewpoint, "").padEnd(9)} ${formatValue(level.dewpointDepression, "").padEnd(8)} ${formatValue(level.windDirection, "", 0).padEnd(7)} ${formatValue(level.windSpeed, "", 0).padEnd(7)} ${levelType}`
      doc.text(row, margin, y)
      y += 6
    })
    ...
    doc.setFont("helvetica", "normal")
    doc.text(
      `Total Levels: ${decodedData.significantLevels.length} | Max Wind: ${Math.max(...decodedData.significantLevels.map((l) => l.windSpeed || 0))} kt`,
      margin,
      y,
    )
  ...
  // Professional Export Button Component
  const ExportButton = ({
    type,
    onExport,
    icon: Icon,
      ...
      <div className="container mx-auto p-6 space-y-6">
        <div className="text-center space-y-4 py-8">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Advanced meteorological data decoder for TTAA/TTBB upper air soundings with multi-format file support
          </p>
        </div>
              ...
              >
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">Supports TXT, CSV, PDF files up to 10MB each</p>
                  </div>
                </div>
                <Input
                  id="file-input"
              ...
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                <Card className="border-0 shadow-lg bg-gradient-to-br from-blue-50 to-blue-100 border-l-4 border-l-blue-500">
                  ...
                  <CardContent>
                    ...
                    <p className="text-xs text-blue-700">{String(decodedData.time).padStart(2, "0")}:00 UTC</p>
                  </CardContent>
                </Card>
                ...
                <Card className="border-0 shadow-lg bg-gradient-to-br from-red-50 to-red-100 border-l-4 border-l-red-500">
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold text-red-900">{decodedData.surfacePressure} mb</div>
                ...
                <Card className="border-0 shadow-lg bg-gradient-to-br from-green-50 to-green-100 border-l-4 border-l-green-500">
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold text-green-900">{decodedData.surfaceWindDirection}°</div>
                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="mt-4 text-sm text-gray-600 bg-green-50 p-3 rounded-lg border border-green-200">
                      <div className="flex items-center gap-2">
                        ...
                        <span>
                          Showing {decodedData.mandatoryLevels.filter((level) => level.pressure >= 100).length}{" "}
                          mandatory levels (≥100 mb pressure)
                        </span>
                      </div>
                      {decodedData.mandatoryLevels.filter((level) => level.pressure < 100).length > 0 && (
                        <div className="flex items-center gap-2 mt-1">
                          <div className="w-2 h-2 bg-gray-400 rounded-full"></div>
                          <span className="text-gray-500">
                            {decodedData.mandatoryLevels.filter((level) => level.pressure < 100).length} levels below
                            100 mb filtered out
                          </span>
                        </div>
                      )}
                    ...
                    <CardContent className="pt-6">
                      {decodedData.tropopause ? (
                        <div className="space-y-4">
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            <div className="bg-gradient-to-br from-blue-100 to-blue-200 p-3 rounded-lg border border-blue-300">
                              ...
                              <div className="text-lg font-bold text-blue-800">{decodedData.tropopause.pressure}</div>
                            ...
                            <div className="bg-gradient-to-br from-purple-100 to-purple-200 p-3 rounded-lg border border-purple-300">
                              ...
                              <div className="text-lg font-bold text-purple-800">
                                ~{Math.round(44330 * (1 - Math.pow(decodedData.tropopause.pressure / 1013.25, 0.1903)))}
                              </div>
                        ...
                        <div className="text-center py-8">
                          ...
                          <p className="text-sm text-gray-500 mt-1">Look for 88PmPmPm format in TTAA data</p>
                        </div>
                      )}
                    </CardContent>
                  </Card>
                    ...
                    <CardContent className="pt-6">
                      {decodedData.maxWind ? (
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="bg-gradient-to-br from-purple-100 to-purple-200 p-4 rounded-lg border border-purple-300">
                              <div className="flex items-center justify-between mb-2">
                                <span className="text-sm font-medium text-purple-700">Pressure Level</span>
                                <div className="w-3 h-3 bg-purple-500 rounded-full"></div>
                              </div>
                              <div className="text-2xl font-bold text-purple-800">{decodedData.maxWind.pressure}</div>
                            ...
                            <div className="bg-gradient-to-br from-green-100 to-emerald-200 p-4 rounded-lg border border-green-300">
                              <div className="flex items-center justify-between mb-2">
                                <span className="text-sm font-medium text-green-700">Wind Direction</span>
                              ...
                              <div className="text-xs text-green-600">
                                {decodedData.maxWind.windDirection >= 337.5 || decodedData.maxWind.windDirection < 22.5
                                  ? "from N"
                            ...
                            <div className="bg-gradient-to-br from-orange-100 to-red-200 p-4 rounded-lg border border-orange-300">
                              <div className="flex items-center justify-between mb-2">
                                <span className="text-sm font-medium text-orange-700">Wind Speed</span>
                                <div className="w-3 h-3 bg-orange-500 rounded-full"></div>
                              </div>
                              <div className="text-2xl font-bold text-orange-800">{decodedData.maxWind.windSpeed}</div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                              ...
                              <div className="space-y-2">
                                <div className="flex justify-between">
                                  ...
                                  <span className="text-blue-800 font-medium">Max wind indicator</span>
                                </div>
                                <div className="flex justify-between">
                        ...
                        <div className="text-center py-8">
                          ...
                          <p className="text-sm text-gray-500 mt-1">Look for 77PmPmPm dddff format in TTAA data</p>
                        </div>
                      )}
                    </CardContent>
                  </Card>
                ...
                <CardContent className="p-6">
                  {decodedData.significantLevels.length > 0 ? (
                                ...
                                {decodedData.significantLevels
                                  ...
                                  .map((level, index) => (
                                    ...
                                    >
                                      ...
                                      <td className="p-3">{formatValue(level.temperature, "")}</td>
                                      <td className="p-3">{formatValue(level.dewpoint, "")}</td>
                                      <td className="p-3">{formatValue(level.dewpointDepression, "")}</td>
                                      <td className="p-3">
                                        <Badge
                                          variant="outline"
                                          className={`text-xs ${
                                        ...
                                        >
                                          {level.pressure === decodedData.surfacePressure ? "Surface" : "Significant"}
                                        </Badge>
                                      </td>
                                    </tr>
                                  ))}
                          ...
                          <div className="mt-4 text-sm text-gray-600 bg-red-50 p-3 rounded-lg border border-red-200">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                              <div className="flex items-center gap-2">
                                ...
                                <span>
                                  Temperature levels:{" "}
                                  {decodedData.significantLevels.filter((l) => l.temperature !== null).length}
                                </span>
                              </div>
                              <div className="flex items-center gap-2">
                                ...
                                <span>
                                  Dewpoint levels:{" "}
                                  {decodedData.significantLevels.filter((l) => l.dewpoint !== null).length}
                                </span>
                              </div>
                              <div className="flex items-center gap-2">
                                ...
                                <span>
                                  Surface included:{" "}
                                  {decodedData.significantLevels.some((l) => l.pressure === decodedData.surfacePressure)
                                    ? "Yes"
                                    : "No"}
                                </span>
                              </div>
                            ...
                            <table className="w-full text-sm">
                              <thead>
                                <tr className="border-b-2 border-blue-200 bg-gradient-to-r from-blue-100 to-cyan-100">
                                  ...
                                  <th className="text-left p-3 font-semibold text-blue-800">Wind Direction (°)</th>
                                  <th className="text-left p-3 font-semibold text-blue-800">Wind Speed (kt)</th>
                                  <th className="text-left p-3 font-semibold text-blue-800">Cardinal Direction</th>
                                        ...
                                        {level.windDirection !== null ? (
                                          <span className="text-xs font-medium text-gray-600">
                                            {level.windDirection >= 337.5 || level.windDirection < 22.5
                                              ? "N"
                                              : level.windDirection >= 22.5 && level.windDirection < 67.5
                                                ? "NE"
                                                : level.windDirection >= 67.5 && level.windDirection < 112.5
                                                  ? "E"
                                                  : level.windDirection >= 112.5 && level.windDirection < 157.5
                                                    ? "SE"
                                                    : level.windDirection >= 157.5 && level.windDirection < 202.5
                                                      ? "S"
                                                      : level.windDirection >= 202.5 && level.windDirection < 247.5
                                                        ? "SW"
                                                        : level.windDirection >= 247.5 && level.windDirection < 292.5
                                                          ? "W"
                                                          : "NW"}
                                          </span>
                                        ) : (
                          ...
                          <div className="mt-4 text-sm text-gray-600 bg-blue-50 p-3 rounded-lg border border-blue-200">
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                              ...
                              <div className="flex items-center gap-2">
                                <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                                <span>
                                  Max speed: {Math.max(...decodedData.significantLevels.map((l) => l.windSpeed || 0))}{" "}
                                  kt
                                </span>
                              </div>
                              <div className="flex items-center gap-2">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span>
                                  ...
                                  {decodedData.significantLevels.some(
                                    (l) => l.pressure === decodedData.surfacePressure && l.windSpeed !== null,
                                  )
                                    ? "Yes"
                                    : "No"}
                                </span>
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-sm text-gray-500">Upload a file containing TTBB data or enter it manually</p>
                    </div>
                  )}
                </CardContent>
              </Card>
