...
interface WeatherObservation {
  ...
  rainfallTimeSlots: RainfallTimeSlot[];
    ...
    const exportToCSV = () => {
      ...
      const rows = mergedData.map((record) => {
        ...
        return [
          ...
          record.utcTime
            ? format(new Date(record.utcTime), "yyyy-MM-dd")
            : "--",
      ...
      link.setAttribute(
        "download",
        `merged_meteorological_data_${startDate}_to_${endDate}.csv`,
      );
...
`;
      ...
      mergedData.forEach((record, index) => {
        const metEntry =
          record.meteorologicalEntry || ({} as MeteorologicalEntry);
        const weatherObs =
          record.weatherObservation || ({} as WeatherObservation);
        ...
        txtContent += `Rainfall Time Start${" ".repeat(2)} ---> ${
          weatherObs?.rainfallTimeSlots &&
          weatherObs.rainfallTimeSlots.length > 0
            ? weatherObs.rainfallTimeSlots
                .map((slot, index) => `Slot ${index + 1}: ${slot.timeStart}`)
...
${"=".repeat(60)}`;
    ...
    const mergeData = (
      firstData: ObservingTimeEntry[],
      secondData: ObservationData[],
    ) => {
      ...
      firstData.forEach((observingTime) => {
        observingTime.MeteorologicalEntry.forEach((metEntry) => {
          ...
          const matchingSecondCard = secondData.find(
            (obs) =>
              obs.id === observingTime.id ||
              obs.utcTime === observingTime.utcTime,
          );
      ...
      secondData.forEach((obsData) => {
        if (
          !processedObservingTimes.has(obsData.id) &&
          obsData.WeatherObservation?.length > 0
        ) {
                  ...
                  <thead>
                    ...
                    <tr>
                      ...
                      <th
                        colSpan={5}
                        rowSpan={2}
                        className="border border-slate-300 bg-gradient-to-b from-emerald-50 to-emerald-100 p-1 font-medium text-emerald-800"
                      >
                        RAINFALL
                      </th>
                      <th
                  ...
                  <tbody>
                    {loading ? (
                      ...
                      mergedData.map((record, index) => {
                        ...
                        const cloudClass = getCloudStatusColor(
                          weatherObs?.totalCloudAmount || null,
                        );
                        const rowClass =
