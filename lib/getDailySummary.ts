...
export const generateDailySummary = (
  ...
  // Calculate rain duration - handle both new (timeSlots) and old format
  const totalRainDuration = todayWeatherObservations.reduce((totalMinutes: number, item: any) => {
    // Check for new format (timeSlots array)
    if (item.rainfallTimeSlots && Array.isArray(item.rainfallTimeSlots)) {
      const slotsDuration = item.rainfallTimeSlots.reduce((slotTotal: number, slot: any) => {
        if (slot.timeStart && slot.timeEnd) {
          ...
          return slotTotal + duration;
        }
        return slotTotal;
      }, 0);
      
      return totalMinutes + slotsDuration;
    }
